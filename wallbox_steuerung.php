...
//
//  Pro Solaranzeige Raspberry kann nur eine Wallbox angeschlossen werden!
//  ----------------------------------------------------------------------
*****************************************************************************/
//
//
...
for ($i=1; $i < 7; $i++) {
  ...
  if (file_exists($Pfad."/".$i.".wallbox.steuerung.ini"))   {
    log_schreiben("Die Konfig Datei '".$i.".wallbox.steuerung.ini' ist vorhanden ","|- ",3);
    ...
    if (!isset($INI["PV-Quelle"]["BisSOC"]) or !isset($INI["Batterie-Quelle"]["BisSOC"]) or !isset($INI["Netz-Quelle"]["BisSOC"])) {
      log_schreiben("Die INI Datei konnte nicht richtig ausgelesen werden.","|- ",1);
      log_schreiben(print_r($INI,1),"|- ",1);
      goto Ausgang;
    }
    ...
    if ($wbSteuerung["wbSteuerung1"] == 0 and $wbSteuerung["wbSteuerung2"] == 0 and $wbSteuerung["wbSteuerung3"] == 0) {
      log_schreiben("Keine Ladung anstehend. Ausgang...","",2);
      goto Ausgang;
    }
    ...
    elseif (isset($INI["Allgemein"]["Phasen"]) and $INI["Allgemein"]["Phasen"] == 2) {
      log_schreiben("Phasen: ".$INI["Allgemein"]["Phasen"],"",4);
      $PUI = (400*1.41);  //  2 Phasen Berechnung
    }
    else {
    ...
    if (isset($INI["PV-Quelle"]["NurBeiSonne"]) and $INI["PV-Quelle"]["NurBeiSonne"] == 1) {
      log_schreiben("Ladung nur bei genügend Sonnenenergie","",2);
      $NurBeiSonne = true;
    }
    else {
      $NurBeiSonne = false;
    ...
    if (isset($INI["PV-Quelle"]["Eigenverbrauch"])) {
      ...
      log_schreiben("Eigenverbrauch geschätzt: ".$Eigenverbrauch." Watt","",3);
    }
    else {
      $Eigenverbrauch = 0;
    }
    ...
    if (!isset($rc["JSON_Ausgabe"]["results"][0]["series"])) {
      ...
      log_schreiben("Unterbrechung Flag aufgehoben.","",3);  
    }
    else {
    ...
    **************************************************************************/
    ...
    //  $Solarleistung = 990;  // Nur zum Testen
    ...
    ************************************************************************/
    ...
    switch ($wbRegler) {
       ...
       case 29:
         ...
         //  Ladestatus 4 = Ladung beendet
         //  Ladestatus 5 = Ladung unterbrochen
         $RFID_Karte = $DB4["RFID_Karte"];
         $Ladestatus = $DB4["Stationsstatus"];
         $Zugangskontrolle = $DB4["Zugangskontrolle"];
         ...
         elseif(($StromL1 > 1 and $StromL2 > 1) or ($StromL1 > 1 and $StromL3 > 1) or ($StromL2 > 1 and $StromL3 > 1)) {
           $PUI= (400*1.41);        //  Wechselstromberechnung 2 Phasen
         }
         else{
    ...
    **************************************************************************/
    ...
    if ($Ladestatus == 1) {
      log_schreiben("Kein Auto angeschlossen. Ausgang....","",2);
      goto Ausgang;
    }
    ...
    log_schreiben("SOC: ".$SOC."%","",3);
    log_schreiben("Ladestatus: ".$Ladestatus,"",3);
    log_schreiben("LadungAktiv: ".$LSAktiv,"",3);
    log_schreiben("MaxAmpere: ".$MaxAmpere,"",3);
    log_schreiben("Pause: ".$Pause,"",3);
    log_schreiben("Kabelstatus: ".$Kabelstatus,"",3);
    log_schreiben("Inselanlage: ".var_dump($Inselanlage)." ","",3);
    ...
    *************************************************************************/
    ...
    if ($wbSteuerung["wbSteuerung1"] == 1) {  // Start der PV Ladung
      log_schreiben("Ladestrom (MinMilliAmpere): ".$INI["PV-Quelle"]["MinMilliAmpere"],"",3);
      log_schreiben("Ladequelle PV-Module.","",2);
      if ($Kabelstatus == 7) {
        log_schreiben("Kabel ist angeschlossen und verriegelt. Ladung kann beginnen.","",3);
        ...
        if ($Ladestatus == 3)  {              // Es wird geladen.
          log_schreiben("Ladung aktiv. Aktuelle Ladung: ".$WhLadung." Wh","",2);


          if ($SOC <= $INI["PV-Quelle"]["BisSOC"] or ( $NurBeiSonne == true and $Solarleistung <= $Eigenverbrauch)) {
            log_schreiben("Unterbrechung wegen zu geringer Solarleistung oder SOC.","",3);
            $ret = befehl_senden("Pause",$wbRegler,$wbGeraeteNummer,0);
            if ($ret == true) {
              log_schreiben("Ladungspause. SOC kleiner ".$INI["PV-Quelle"]["BisSOC"]."% oder keine Sonne mehr.","",3);
            ...
            if (date("i") % 2 == 0) {    // Alle 2 Minuten. Kann aber in "% 1"  oder "% 5" geändert werden.
              ...
              if ($MaxAmpere <> $Ladestrom) {
                ...
                if ($ret == true) {
                  ...
                  log_schreiben("Stromänderung,$wbRegler,$wbGeraeteNummer,".$Ladestrom,"",3);
                }
                if ($Pause == 1) {
                  // Flag setzen in die Datenbank "Steuerung"  Measurement "Ladung"  Flag "unterbrechung"
                  $query = "Ladung Unterbrechung=0";
          ...
          elseif (date("i") % 1 == 0) {    // % 4 = alle 4 Minuten Prüfen ob die Ladung geändert werden muss.
            ...
            if ($MaxAmpere <> $Ladestrom) {
              log_schreiben("MaxAmpere Ladestrom: ".$MaxAmpere,"",3);          
              log_schreiben("Eingestellter Ladestrom: ".$Ladestrom,"",3);          
              // Ändern nur wenn der Strom nicht schon eingestellt ist.
              $ret = befehl_senden("Stromaenderung",$wbRegler,$wbGeraeteNummer,$Ladestrom);
              if ($ret == true) {
                log_schreiben("Stromänderung. ".$Ladestrom." MaxAmpere: ".$MaxAmpere,"",3);
            ...
            log_schreiben("Aktueller Ladestrom: ".$Ladestrom,"",3);          
          }
        }
        ...
        elseif ($Ladestatus == 4)  {   // Ladung wurde beendet.
          log_schreiben("Ladung vom Fahrzeug beendet. Aktuelle Solarleistung: ".$Solarleistung." Watt.","",2);
        }
        elseif ($Ladestatus == 2) {  // Ladestatus 2
          // Sind die Voraussetzungen erfüllt?
          if ($INI["PV-Quelle"]["Sonnenaufgang"] == 1 and date("H:i") > date("H:i",($Sonnenaufgang + 7200)) )  {
          ...
          if ($Solarleistung >= $INI["PV-Quelle"]["MinSolarleistung"] and $StartLadung == true)  {
            ...
            if ( $SOC >= $INI["PV-Quelle"]["AbSOC"] )  {
              ...
              if ($ret == true) {
                ...
                log_schreiben("Start,$wbRegler,$wbGeraeteNummer,".$INI["PV-Quelle"]["MinMilliAmpere"],"",3);
              ...
              if ($INI["PV-Quelle"]["MaxEnergie"] > 0) {
                ...
                if ($ret == true) {
                  ...
                  log_schreiben("MaxEnergie,$wbRegler,$wbGeraeteNummer,".$INI["PV-Quelle"]["MaxEnergie"],"",3);
                }
              }
              else {
              ...
              log_schreiben("Die Batteriekapazität ist nicht ausreichend: ".$SOC." !","",3);
            }
          }
          else {
            log_schreiben("Die Minimale Solarleistung ist noch nicht vorhanden: ".$INI["PV-Quelle"]["MinSolarleistung"],"",3);
          }
        }
        else {
      ...
      elseif ($Ladestatus == 4 and $Pause == 0)  {   // Ladung wurde beendet.
        ...
        log_schreiben("Kabel ist nicht angeschlossen.","",2);
        goto Ausgang;
     }
    }
    ...
    *************************************************************************/
    ...
    if ($wbSteuerung["wbSteuerung2"] == 1) {
      log_schreiben("Ladestrom (MinMilliAmpere): ".$INI["Batterie-Quelle"]["MinMilliAmpere"],"",3);
      ...
      if ($Kabelstatus == 7) {
        ...
        if ($Ladestatus == 3)  {   // Es wird geladen.
          log_schreiben("Ladung aktiv. Aktuelle Ladung: ".$WhLadung." Wh","",2);
          if ($SOC <= $INI["Batterie-Quelle"]["BisSOC"])  {
            $ret = befehl_senden("Pause",$wbRegler,$wbGeraeteNummer,0);
            if ($ret == true) {
          ...
          if (date("i") % 4 == 0) {
            ...
            if ($Ladestrom > 0) {
              ...
              if ($ret == true) {
                ...
                log_schreiben("Stromäbderung,$wbRegler,$wbGeraeteNummer,".$Ladestrom,"",3);
              }
            }
            log_schreiben("Aktueller Ladestrom: ".$Ladestrom,"",3);          
          }
        ...
        elseif ($Ladestatus == 2) {
          // Sind die Voraussetzungen erfüllt?
          if ($INI["Batterie-Quelle"]["Sonnenaufgang"] == 1 and date("H:i") > date("H:i",($Sonnenaufgang + 7200)) )  {
            log_schreiben("Es ist tagsüber, mehr als 1 Stunde nach Sonnenaufgang.","",3);
            $StartLadung = true;
          }
          ...
          if ($SOC >= $INI["Batterie-Quelle"]["AbSOC"] and $StartLadung == true)  {
            ...
            if ($ret == true) {
              ...
              log_schreiben("Start,$wbRegler,$wbGeraeteNummer,".$INI["Batterie-Quelle"]["MaxMilliAmpere"],"",3);
            }
            if ($INI["Batterie-Quelle"]["MaxEnergie"] > 0)   {
              $ret = befehl_senden("MaxEnergie",$wbRegler,$wbGeraeteNummer,($INI["Batterie-Quelle"]["MaxEnergie"]));
              if ($ret == true) {
                log_schreiben("Maximale Energiemenge setzen. ","",3);
                log_schreiben("MaxEnergie,$wbRegler,$wbGeraeteNummer,".$INI["Batterie-Quelle"]["MaxEnergie"],"",3);
    ...
    *************************************************************************/
    ...
    if ($wbSteuerung["wbSteuerung3"] == 1) {
      log_schreiben("Ladestrom (MinMilliAmpere): ".$INI["Netz-Quelle"]["MinMilliAmpere"],"",3);
      ...
      if ($Kabelstatus == 7) {
        ...
        elseif ($Ladestatus == 2) {
          ...
          if ($INI["Netz-Quelle"]["Sonnenaufgang"] == 1 and date("H:i") > date("H:i",($Sonnenaufgang + 7200)) )  {
            log_schreiben("Es ist tagsüber, mehr als 1 Stunde nach Sonnenaufgang.","",3);
            $StartLadung = true;
          }
          ...
          if ($SOC >= $INI["Netz-Quelle"]["AbSOC"] and $StartLadung == true)  {
            ...
            if ($ret == true) {
              ...
              log_schreiben("Start,$wbRegler,$wbGeraeteNummer,".$INI["Netz-Quelle"]["MaxMilliAmpere"],"",3);
            ...
            if ($INI["PV-Quelle"]["MaxEnergie"] > 0) {
              ...
              if ($ret == true) {
                log_schreiben("Maximale Energiemenge setzen. ","",3);
                log_schreiben("MaxEnergie,$wbRegler,$wbGeraeteNummer,".$INI["Netz-Quelle"]["MaxEnergie"],"",3);
              }
            }
            else {
                log_schreiben("Option Maximale Energiemenge ausgeschaltet.","",3);
